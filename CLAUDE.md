...
- In interactive sessions, do not work around it by prefixing every command
  with `pipenv run`. Instead, stop and ask the user to restart the agent from
  within the activated virtualenv.
- In non-interactive/autonomous sandboxed sessions (such as GitHub Copilot
  Coding Agent), proceed with whichever invocation is most reliable (including
  `pipenv run ...`) so repo commands can run without blocking on manual restart.
