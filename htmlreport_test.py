...
class TestGenerateReport(unittest.TestCase):
    ...
    def test_club_page_consolidated_and_per_team(self):
        ...
        consolidated_table = harrow_page.split('<h2 id="')[0]
        ...
        harrow1_section = harrow_page.split('id="harrow-1"')[1].split("<h2")[0]
        self.assertEqual(harrow1_section.count("<tr>"), 4)  # header + 3 fixtures

    def test_club_page_team_headings_have_anchor_links(self):
        harrow_page = (self.output_dir / "club-harrow.html").read_text()
        self.assertIn('<a class="anchor-link" href="#harrow-1">§</a>', harrow_page)
        self.assertIn('<a class="anchor-link" href="#harrow-2">§</a>', harrow_page)
...
class TestExcludedFixturesInReport(unittest.TestCase):
    ...
    def test_club_page_consolidated_shows_excluded_fixture(self):
        ...
        consolidated_table = ealing_page.split('<h2 id="')[0]
        self.assertIn("TBC", consolidated_table)

    def test_team_page_shows_excluded_fixture_with_blank_days_since(self):
        harrow_page = (self.output_dir / "club-harrow.html").read_text()
        harrow2_section = harrow_page.split('id="harrow-2"')[1].split("<h2")[0]
        expected_row = (
